...
from example_varats.reports.test_report import MyTestReport
    ...
    def analyze(self) -> actions.StepResult:
        ...
        for binary in self.project.binaries:
            result_file = create_new_success_result_filepath(
                self.__experiment_handle, MyTestReport, self.project, binary,
                config_id)

            print(f"pew pew {result_file=}")
            run_cmd = touch[f"{result_file}"]

            exec_func_with_pe_error_handler(
                run_cmd,
                create_default_analysis_failure_handler(
                    self.__experiment_handle, self.project, MyTestReport))
...
class JustCompileTest(VersionExperiment, shorthand="JCT"):
    ...
    REPORT_SPEC = ReportSpecification(MyTestReport)
